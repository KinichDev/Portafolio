...
HTMLElement.prototype.displayNone = function () {
    if (this.style.display === "none") return this; // Evita reejecutar si ya está oculto
    this.style.animation = "fadeOut 1s forwards"; // Agrega una animación de desvanecimiento
    this.style.opacity = "1"; // Asegura que la opacidad inicial sea 1
    setTimeout(() => {
        ...
        this.style.opacity = ""; // Limpia la opacidad para evitar conflictos
        return this;
...
HTMLElement.prototype.displayFlex = function () {
    if (this.style.display === "flex") return this; // Evita reejecutar si ya está visible
    this.style.display = "flex"; // Cambia el display a "flex" antes de la animación
    this.style.animation = "fadeIn 0.3s forwards"; // Agrega una animación de aparición
    this.style.opacity = "0"; // Asegura que la opacidad inicial sea 0
    setTimeout(() => {
        this.style.animation = ""; // Limpia la propiedad de animación
        this.style.opacity = ""; // Limpia la opacidad para evitar conflictos
        return this;
    }, 300); // Tiempo suficiente para que la animación termine
};

HTMLElement.prototype.generateBoxes = function(count) {
    ...
    style.innerHTML = `
      ...
      .box {
        ...
        animation: animateBox 5s ease-in-out infinite;
        border-radius: 4px;
        box-shadow: 0 0 9px;
        opacity: .05;
      }
      ...
      @keyframes animateBox {
        ...
        60% {
          transform: translateX(-250px) translateY(230px);
          width: 50px;
          height: 50px;
        }
        75% {
    ...
    function createBox(top, left, delay) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const delay = `${i}s`;
      createBox.call(this, position.top, position.left, delay);
    }
}
...
HTMLElement.prototype.entradaSuave = function () {
    ...
    this.style.animation = "tomasCinematicasLentas 45s ease-in-out infinite";
};
...
HTMLElement.prototype.contenidoColage = function(numColage) {
    const style = document.createElement('style');
    document.head.appendChild(style);
    ...
    for (let i = 0; i < numColage; i++) {
        ...
        // Create and append the "colage" element to the container
        let colage = document.createElement('div');
        colage.classList.add('colage');
        this.appendChild(colage);
    }
...
function img(c,i,src) { // ! CREAR UNA ETIQUETA IMG (clase, id, inner)
    ...
    src=""?true:img.setAttribute("src",src + "?" + new Date().getTime())
...
function mkObjImg(n,img_class,img_id,src_) { // ! CREAR UN OBJETO IMG (nodo, clase, id, recurso imagen)
    ...
    let img_temp = img(img_class,img_id,src_ + "?" + new Date().getTime())
...
function videoLoop(c,i,s,inner) {
    ...
    i=""?true:video.setAttribute("src",s + "?" + new Date().getTime())
    video.autoplay = true;
    video.loop = true;  
    video.muted = true;  
...
function flexWindow(div) {
    ...
    document.addEventListener("mousemove", (e) => {
        ...
        // Calcular las nuevas coordenadas del div
        let newLeft = mouseX - offsetX;
        let newTop = mouseY - offsetY;

        // Obtener las dimensiones del contenedor y del div
        const containerRect = container.getBoundingClientRect();
        const divRect = div.getBoundingClientRect();

        // Asegurarse de que el div no se salga del contenedor
        if (newLeft < 0) newLeft = 0;
        if (newTop < 0) newTop = 0;
        if (newLeft + divRect.width > containerRect.width) newLeft = containerRect.width - divRect.width;
        if (newTop + divRect.height > containerRect.height) newTop = containerRect.height - divRect.height;
